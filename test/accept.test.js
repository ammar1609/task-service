...
describe('__________ accept endpoint __________', () => {

    it('should return status 200 if updated successfully', (done) => {
        chai
            .request(app)
            .put(`/accept/${data.itemId}`)
            .end((err, res) => {
                chai.expect(err).to.equal(null);
                chai.expect(res.body.success).to.equal(true);
                chai.expect(res.body.message).to.equal("Borrowing request accepted.");
